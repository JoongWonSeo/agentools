...
class Tools(ABC):
    ...
    lookup_preview: dict[str, Callable]  # dict of tool name to preview function
...
async def call_function_preview(
    call_request: Function, func_lookup_preview: dict[str, Callable]
):
    """
    ...
    if func_name not in func_lookup_preview or not partial_arguments.strip():
        return
    try:
