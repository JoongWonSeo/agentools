...
from .tools import Tools, call_requested_function, function_tool
...
class ChatGPT(Assistant):
    ...
    async def completion_events(self, call_index: int, tools: Tools | None, parallel_calls: bool, openai_args: dict) -> AsyncIterator[Assistant.Event]:
        '''
        ...
        completion_stream = await openai_chat(stream=True, **openai_args)
        ...
        async for chunk, partial in accumulate_partial(completion_stream):
            ...
            if tools and delta.tool_calls:
                # handle tool calls
                async for tool_event in self.partial_tool_events(message.tool_calls, tools, parallel_calls):
                    yield tool_event
    ...
    async def partial_tool_events(self, tool_calls: list[ToolCall], tools: Tools, parallel_calls: bool) -> AsyncIterator[Assistant.Event]:
        '''
        Generate events from a list of tool calls, yielding each tool call, executing them, and yielding the result. You should override this to customize the tool call handling, also defining your own events to yield.
        '''
...
class StructGPT(ChatGPT):
    ...
    def __init__(self, struct: BaseModel, model: str = 'gpt-3.5-turbo', tool_name: str = None):
        @function_tool(name=tool_name, require_doc=False, json_schema=struct.model_json_schema())
    ...
    async def __call__(self, prompt: str, max_attempts: int = 5, model: Optional[str] = None, **openai_kwargs) -> BaseModel:
